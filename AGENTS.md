...
mean both ends always agree, so no struct is versioned, no field is negotiated, and nothing has
to stay readable by an older peer. Per-struct versioning and a compatibility matrix are the
eventual answer, deliberately deferred until the protocol has settled; adopting them now would
handcuff a design still in motion. So treat "just relax the version pinning" or "add a version
field to the protocol" as **premature** unless asked — and where a cross-version read genuinely
happens anyway (an outgoing Probe Agent answering mid-handover), keep it tolerant rather than
...
**Any type that travels between `mdbg` and TypeScript is defined in Rust and translated by
ts-rs. Never hand-write the TypeScript side.** One definition, one source of truth.

The reason this rule needs stating is that breaking it does not fail any build. The wire is
JSON, so two definitions that disagree still compile on both sides — the mismatch shows up at
runtime as a field that is `undefined`, in whichever code path happens to read it. A generated
type turns that into a compile error in the editor instead.

**How to apply:**

1. On the Rust struct/enum, add `ts_rs::TS` to the derives and
   `#[ts(export, export_to = "proxy-protocol/")]` (or `serial-helper/`, `dasm-helper/`).
   **Every nested type needs the same treatment** — ts-rs will not export a struct whose field
...
The generated files are checked in, marked DO NOT EDIT, and strict-type-checked. Edit the Rust
and regenerate; never patch them.

**One thing generation cannot express.** A `#[serde(default)]` Rust field becomes a **required**
TS field, because the generated type describes what the current Rust *emits*. That is not the
same as what every peer *sends*: a long-lived process can be answering from an older build — the
Probe Agent does exactly this while draining during a handover — and `serde(default)` is what
keeps such a reply parsing at all. Where a message can arrive from an older version, read it
defensively (`x.exe?.field`) despite the type, and say so in a comment. The type states the
contract, not the guarantee.

**When to do it.** At the moment TypeScript first reads the type, not later. The funnel protocol
was exported from the start while the admin channel was not — correctly, since nothing in TS
consumed an admin reply for a long time. The first consumer arrived with
`mcu-debug-proxy.proxyStatus` and was written against hand-rolled interfaces, which is precisely
the drift this rule exists to prevent. Export it as you add the consumer.
