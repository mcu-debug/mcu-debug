...
export class BMPServerController extends EventEmitter implements GDBServerController {
    ...
    public connectCommands(): string[] {
        ...
        const commands: string[] = [`target-select extended-remote ${this.args!.BMPGDBSerialPort}`];
        ...
        if (this.args.interface === "jtag") {
            // TODO: handle ctag in when this server supports it
    ...
    public launchCommands(): string[] {
        const commands = [...genDownloadCommands(this.args, []), 'interpreter-exec console "SoftwareReset 1"'];
        return commands;
    }
    ...
    public resetCommands(): string[] {
        const commands: string[] = ['interpreter-exec console "SoftwareReset"'];
        return commands;
    }
