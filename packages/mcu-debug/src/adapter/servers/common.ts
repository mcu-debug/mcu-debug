...
export interface HostConfig {
    ...
    /** Settings for `type: "ssh"`, grouped so the SSH-only knobs stay together and the
     *  schema can require a host whenever any of them is set. Ignored for other types. */
    ssh?: {
        /** SSH host or ~/.ssh/config alias. */
        host: string;
        /** Daemon mode: port a Probe Agent is already listening on at the SSH host. */
        proxyPort?: number;
        /** Token of that pre-running agent. Required with `proxyPort`, ignored otherwise. */
        token?: string;
        /** Pre-installed mcu-debug on the SSH host; skips binary deployment. */
        serverPath?: string;
    };
     ...
     *  the SSH topologies, which need their tunnel established first (see ssh.proxyPort). */
    ...
    pvtProxyToken?: string; // Resolved token: generated for an agent we launch, or taken from ssh.token / proxy.token
