//----------------------------------------------------------
// This file is originally part of https://github.com/llop/gdb-mi-parser
import { GdbMiOutput, GdbMiOutputType, GdbMiRecord, GdbMiRecordType } from "./mi-types";
...
var recordTypeMapping: { [key: string]: GdbMiRecordType } = {
    "^": "result",
    "~": "stream",
    "&": "stream",
    "@": "stream",
...
var outputTypeMapping: { [key: string]: GdbMiOutputType } = {
    "^": "result",
    "~": "console",
    "&": "log",
    "@": "target",
...
function eatWhitespace(line: string, i: number): number {
...
function eatVariableName(line: string, i: number): number {
    while (i < line.length && line[i] != "=") ++i;
    return i + 1;
}

function nextVariableName(line: string, i: number): [number, string] {
    var j = i;
    while (j < line.length && line[j] != "=") ++j;
    return [j, trim(line.substring(i, j))];
}

function nextValue(line: string, i: number): [number, any] | undefined {
    if (line[i] == '"')
        return nextConst(line, i); // parse const
    else if (line[i] == "{")
        return nextTuple(line, i); // parse tuple
    ...
    for (let i = 0; i < line.length; ) {
        if (line[i] === "\\") {
            if (i + 1 < line.length) {
                ...
                const repl = escapedChars[nextChar];
                if (repl !== undefined) {
                    ret += repl;
                    i += 2;
                } else {
...
function nextConst(line: string, i: number): [number, string] {
    var j = i + 1;
    while (j < line.length) {
        if (line[j] == "\\") ++j;
        else if (line[j] == '"') {
...
function nextResult(line: string, i: number): [number, string, any] | undefined {
...
function nextTuple(line: string, i: number): [number, any] {
...
function nextList(line: string, i: number): [number, any] {
    var ret = [];
    while (i < line.length && line[i] != "]") {
...
function nextToken(line: string, i: number): [number, string] {
    var j = i;
    const re = /\d/;
...
function nextClass(line: string, i: number): [number, string] {
    var j = i + 1;
    while (j < line.length && line[j] != ",") ++j;
    return [j, trim(line.substring(i + 1, j))];
}

function parseGdbMiLine(line: string): GdbMiRecord | undefined {
    ...
    return {
        ...
        class: klass,
        result: result,
        prefix: prefix, // Actual character used as prefix
    };
}

export function parseGdbMiOut(data: string): GdbMiOutput {
    const outOfBandRecords: GdbMiRecord[] = [];
    let resultRecord: GdbMiRecord | undefined = undefined;
    let hasTerminator: boolean = false;
