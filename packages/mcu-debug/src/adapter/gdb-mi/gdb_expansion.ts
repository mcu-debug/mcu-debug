import { GdbMiOutput, MINode } from "./mi-types";
...
export function isExpandable(value: string): number {
    ...
    if ((match = nullpointerRegex.exec(value))) {
        return 0;
    }
    if ((match = referenceStringRegex.exec(value))) {
        return 0;
    }
    if ((match = referenceRegex.exec(value))) {
        return 2;
    } // reference
    if ((match = charRegex.exec(value))) {
        return 0;
    }
    if ((match = numberRegex.exec(value))) {
        return 0;
    }
    if ((match = variableRegex.exec(value))) {
        return 0;
    }
    if ((match = errorRegex.exec(value))) {
        return 0;
    }
    return 0;
}
...
export function expandValue(variableCreate: (value: string | object, opts?: { arg: boolean }) => number, value: string, root: string = "", extra?: any): any {
    ...
    function getNamespace(variable: string): string {
        let namespace = "";
        let prefix = "";
        stack.push(variable);
        stack.forEach((name) => {
    ...
    function createValue(name: string, val: any) {
        let ref = 0;
