...
- [x] **2.** Add `serialport` crate with `default-features = false` to `packages/mdbg/Cargo.toml`. Verify Linux build has no libudev dependency (`ldd` the binary, or `cargo tree -e no-dev | grep -i udev`).
