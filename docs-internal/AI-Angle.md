...
> **Config resolution in CLI mode** (no VS Code APIs available) is covered in [cli-config.md](cli-config.md). The short version: `${workspaceFolder}` and `${env:...}` resolve natively; `${config:...}` variables resolve via `.vscode/settings.json`, which the VS Code extension writes automatically as a side-effect of normal settings editing.
