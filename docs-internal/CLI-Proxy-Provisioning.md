...
- **Idle timeout.** When refs reach 0, start a timer (default 5 hours, configurable). Still 0 at expiry → graceful exit, **releasing the serial ports / gdb-servers it was holding**. Any new provision cancels the timer.
