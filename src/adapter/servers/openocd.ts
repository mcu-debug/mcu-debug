...
export class OpenOCDServerController extends EventEmitter implements GDBServerController {
    ...
    public name = "OpenOCD";
    private args: ConfigurationArguments = {} as ConfigurationArguments;
    ...
    public resetCommands(): string[] {
        const commands: string[] = ['interpreter-exec console "monitor reset halt"'];
    ...
    private startRttMonitor() {
        this.session!.gdbInstance!.on("msg", (type, msg) => {
            if (this.rttStarted) {
                return;
            }
        ...
        this.session!.gdbInstance!.on(GdbEventNames.Stopped, async (info: any, reason: string) => {
            if (reason === "entry") {
                return;
            } // Should not happen
    ...
    // If tclSocket is null, it was opened once but then later closed due to error or the other end closed it
    private tclSocket: net.Socket | null | undefined = undefined;
    ...
    private rttPollStart() {
        this.rttPollTimer = setInterval(async () => {
            if (this.session!.gdbInstance!.status === "running" && !this.rttAutoStartDetected) {
                try {
                    this.dbgPollCounter++;
